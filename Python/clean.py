...
# Convert joining_date to proper DATE format, invalid dates to NaT
df['joining_date'] = pd.to_datetime(df['joining_date'], errors='coerce', format='%d-%m-%Y')

# Replace NaT (invalid dates) with NULL
df['joining_date'] = df['joining_date'].fillna(pd.NaT)  # Optional: Use default date like '1900-01-01'
...
try:
    ...
    for _, row in df.iterrows():
        ...
        """, (
            ...
            row['department'], 
            row['salary'], 
            row['joining_date'] if pd.notnull(row['joining_date']) else None,  # Replace NaT with NULL
            row['age']
        ))
